...
class TrackMaker:
    ...
    def get_track_fast(distance):
        track = [[-random.randint(15, 22), -random.randint(22, 25), 0], [0, 0, 0]]
        rand_x = 0
        passtime = 40
        for i in range(10):
    ...
    def choice_track_list(self, dist):
        ...
        if str(dist) in self.t_dict:
            print('in file %s' % dist)
            return self.t_dict[str(dist)], 1
        if str(dist - 1) in self.t_dict:
            print('in file %s-1' % dist)
            return self.t_dict[str(dist - 1)], 1
        if str(dist + 1) in self.t_dict:
            print('in file %s+1' % dist)
            return self.t_dict[str(dist + 1)], 1
        if str(dist - 2) in self.t_dict:
            print('in file %s-2' % dist)
            return self.t_dict[str(dist - 2)], 1
        if str(dist + 2) in self.t_dict:
            print('in file %s+2' % dist)
            return self.t_dict[str(dist + 2)], 1
    ...
    def choice_track(self, dist):
        track, tag = self.choice_track_list(dist)  # 来自训练路径 tag=1
        # 规范化轨迹数据  [[x,y,t],...]
        ...
        if tag != 1:
            # 采用垃圾算法获取轨迹 建议重写
            new_track_list = self.get_track_fast(dist)
        else:
            # tag==1 轨迹数据来自文件 直接赋值
